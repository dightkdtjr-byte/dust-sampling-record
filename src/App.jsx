...
export default function App() {
  ...
  const handleKFactorChange = (e) => {
      ...
      setFormData(prev => {
          const newMeters = prev.gasMeters.map((meter, idx) => {
              ...
              if (!isNaN(kVal) && !isNaN(dpValue)) return { ...meter, pressure: roundDH(kVal * dpValue) };
              return meter;
          });
  ...
  const generateRecommendation = (mode) => {
    ...
    return { bestNozzleNum: bestNozzle.num, finalDn: Dn_best.toFixed(2), fastestTime, calculatedK: K, expectedDH: roundDH(K * dpAvg) };
  };
  ...
  const applyRecommendation = (opt) => {
    setFormData(prev => {
        const newMeters = prev.gasMeters.map((meter, idx) => {
            if (idx === 0) return meter;
            const dpValue = parseFloat(meter.dp);
            if (!isNaN(parseFloat(opt.calculatedK)) && !isNaN(dpValue)) return { ...meter, pressure: roundDH(parseFloat(opt.calculatedK) * dpValue) };
            return meter;
        });
        ...
        return { 
          ...
          kFactor: String(opt.calculatedK), 
  ...
  const getExpectedValues = () => {
    ...
    if (!isNaN(k) && !isNaN(dp)) dH = roundDH(k * dp);
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 md:p-8 font-sans text-slate-800 selection:bg-emerald-100">
        ...
        <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="mt-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-sm font-bold text-slate-800 flex items-center gap-1">
                    <Target className="w-4 h-4 text-emerald-600"/> 굴뚝 내경({formData.stackDiameter}m) 연동 측정점 산출
                  </h3>
                ...
                <div className="mt-4 flex flex-col md:flex-row justify-between items-center gap-3 border-t border-slate-100 pt-4">
                  ...
                  <button type="button" onClick={applySamplingPointsToTable} className="w-full md:w-auto py-2 px-5 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700 shadow-sm flex items-center justify-center gap-2 transition-colors shrink-0 border border-emerald-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               ...
               <div className="lg:col-span-1 flex flex-col gap-4">
                 ...
                 <div className="bg-white p-4 rounded-lg border border-teal-200 shadow-sm relative overflow-hidden">
                    ...
                    <h3 className="text-xs font-bold text-teal-800 mb-3 flex items-center gap-1 border-b border-teal-100 pb-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col h-full">
              ...
              <div className="flex-1 overflow-auto bg-slate-50/50 p-2 rounded border border-slate-200 mb-4">
                ...
                <button type="button" onClick={addPoint} className="w-full mt-2 py-1.5 flex items-center justify-center gap-1 text-emerald-700 bg-emerald-100 hover:bg-emerald-200 rounded text-xs font-bold border border-emerald-300 transition-colors shadow-sm">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="bg-emerald-50 border border-emerald-200 p-5 rounded-lg mb-6 shadow-sm">
               <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-4 gap-2">
                 <h3 className="font-bold text-emerald-800 flex items-center gap-2">
                    <Target className="w-5 h-5 text-emerald-600" /> 1단계: 채취조건 정밀 산출 (기기 고유 계수 연동)
                 </h3>
               ...
               <div className="mb-4">
                  <button type="button" onClick={handleCalculateOptions} className="w-full py-3 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700 transition shadow-md flex justify-center items-center gap-2 border border-emerald-700">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                    <div className="bg-white border-2 border-emerald-300 p-4 rounded-lg shadow-sm hover:border-emerald-500 transition-colors">
                       ...
                       <ul className="text-sm text-slate-700 space-y-2 mb-4">
                          ...
                          <li className="flex justify-between font-bold"><span>K-Factor:</span> <span className="font-black">{Number(recommendations.fast.calculatedK).toFixed(5)}</span></li>
                       ...
                       <button type="button" onClick={() => applyRecommendation(recommendations.fast)} className="w-full py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 shadow-sm flex justify-center items-center gap-1"><CheckCircle2 className="w-4 h-4"/> 적용하기</button>
                    </div>
                    <div className="bg-teal-50 border-2 border-teal-400 p-4 rounded-lg shadow-sm hover:border-teal-500 transition-colors">
                       ...
                       <ul className="text-sm text-slate-700 space-y-2 mb-4">
                          ...
                          <li className="flex justify-between font-bold"><span>K-Factor:</span> <span className="font-black">{Number(recommendations.standard.calculatedK).toFixed(5)}</span></li>
                       ...
                       <button type="button" onClick={() => applyRecommendation(recommendations.standard)} className="w-full py-2 bg-teal-600 text-white rounded-lg text-sm font-bold hover:bg-teal-700 shadow-sm flex justify-center items-center gap-1"><CheckCircle2 className="w-4 h-4"/> 적용하기</button>
                    </div>
                    <div className="bg-cyan-50 border-2 border-cyan-300 p-4 rounded-lg shadow-sm hover:border-cyan-500 transition-colors">
                       <h4 className="font-bold text-cyan-900 mb-3 flex items-center gap-1 border-b border-cyan-200 pb-2">🛡️ 안정/장시간 <span className="text-[10px] font-bold text-cyan-600 ml-auto">(ΔH 10 내외)</span></h4>
                       <ul className="text-sm text-slate-700 space-y-2 mb-4">
                          ...
                          <li className="flex justify-between font-bold"><span>K-Factor:</span> <span className="font-black">{Number(recommendations.stable.calculatedK).toFixed(5)}</span></li>
                       ...
                       <button type="button" onClick={() => applyRecommendation(recommendations.stable)} className="w-full py-2 bg-cyan-600 text-white rounded-lg text-sm font-bold hover:bg-cyan-700 shadow-sm flex justify-center items-center gap-1"><CheckCircle2 className="w-4 h-4"/> 적용하기</button>
                    </div>
                 </div>
               )}
               ...
               <details className="mt-4 mb-4">
                  <summary className="cursor-pointer text-xs font-bold text-emerald-700 hover:text-emerald-900 flex items-center gap-1">
                     ...
                     <table className="w-full text-xs text-center border-collapse border border-emerald-300">
                       ...
                       <tbody className="bg-white">
                          {NOZZLE_SET.map((n) => {
                             ...
                             return (
                               <tr key={n.num} className={isSelected ? "bg-emerald-100 font-bold" : "hover:bg-emerald-50"}>
                                 ...
                                 <td className="border border-emerald-200 p-1 font-bold">{n.d.toFixed(2)}</td>
               ...
               <div className="bg-white p-4 border border-emerald-200 rounded-lg shadow-inner">
                  ...
                  <div className="flex flex-wrap justify-center gap-2 text-center">
                      ...
                      <div className="bg-emerald-50 py-2 px-1 rounded-lg border border-emerald-200 flex-1 min-w-[90px]">
                          ...
                          <div className="font-black text-emerald-900 mt-1">{formData.kFactor ? Number(formData.kFactor).toFixed(5) : '-'}</div>
                      </div>
            ...
            <div className="mb-6">
              ...
              <button type="button" onClick={addGasMeter} className="w-full mt-2 py-2 flex items-center justify-center gap-1 text-emerald-700 bg-emerald-100 hover:bg-emerald-200 rounded-lg text-sm font-bold border border-emerald-300 transition-colors shadow-sm">
            ...
            <div className="bg-slate-800 p-5 rounded-2xl flex flex-col justify-between text-white shadow-lg border border-slate-700">
              <div className="flex items-center gap-3 mb-4">
                <Gauge className="w-8 h-8 text-emerald-400" />
                <div>
                  <h3 className="font-bold text-lg text-emerald-300 tracking-tight">전체 평균 등속흡인율 (Isokinetic Rate)</h3>
                  <p className="text-xs text-slate-400 mt-1">대기오염공정시험기준 유효범위 : 95% ~ 105% (수식 0.00346 적용됨)</p>
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-4 mt-2">
                <div className="bg-slate-700 p-4 rounded-xl border border-slate-600 flex flex-col items-center justify-center">
                  <span className="text-xs text-slate-300 mb-1 font-bold">사전 수분량 기준 (전체)</span>
                  <div className="text-2xl font-bold text-slate-100">{calcIsokineticRate(false)}<span className="text-sm ml-1 text-slate-400">%</span></div>
                </div>
                <div className="bg-emerald-600 p-4 rounded-xl border border-emerald-500 flex flex-col items-center justify-center shadow-md relative overflow-hidden">
                  ...
                  <div className="text-4xl font-black text-white">{calcIsokineticRate(true)}<span className="text-xl ml-1 text-emerald-200">%</span></div>
                </div>
              </div>
                ...
                {calcIsokineticRate(true) !== '-' && (
                  <div>
                    {(parseFloat(calcIsokineticRate(true)) >= 95 && parseFloat(calcIsokineticRate(true)) <= 105) ? 
                      <span className="flex items-center text-emerald-900 text-sm font-bold bg-emerald-400 px-4 py-2 rounded-full shadow-sm"><CheckCircle2 className="w-4 h-4 mr-1"/> 적합 (최종 유효 데이터)</span> : 
                      <span className="flex items-center text-white text-sm font-bold bg-red-500 px-4 py-2 rounded-full shadow-sm"><AlertTriangle className="w-4 h-4 mr-1"/> 부적합 (재측정 요망)</span>
                    }
                  </div>
                )}
              </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md text-white mb-6 relative overflow-hidden">
               <h3 className="text-sm font-bold text-slate-300 mb-4 flex items-center gap-2 border-b border-slate-600 pb-3">
                 <FileSpreadsheet className="w-4 h-4"/> 최종 산출 결과 요약
               </h3>
               <div className="grid grid-cols-2 md:grid-cols-4 gap-6 divide-y md:divide-y-0 md:divide-x divide-slate-600 text-center">
          ...
          <div className="flex items-center justify-center gap-4 mt-8 pb-8 border-b-2 border-slate-200 border-dashed">
            <button type="button" onClick={() => window.location.reload()} className="px-8 py-3.5 bg-white border border-slate-300 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors flex items-center gap-2 shadow-sm">
              <RefreshCw className="w-5 h-5" /> 새 기록지 작성
            </button>
            <button type="submit" className="px-8 py-3.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors flex items-center gap-2 shadow-md">
        ...
        {savedData.length > 0 && (
          <div className="bg-slate-800 p-6 rounded-2xl shadow-lg mt-8 text-white border border-slate-700">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-700 pb-4">
              <h2 className="text-xl font-bold flex items-center gap-2 text-emerald-400"><FileSpreadsheet className="w-6 h-6"/> 저장된 종합 리포트</h2>
              <button 
                onClick={exportToCSV} 
                className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-colors shadow-md text-sm"
              >
              ...
              <table className="w-full text-sm text-center">
                <thead className="bg-slate-700 text-slate-300">
                  <tr>
                    ...
                    <th className="p-3 font-bold text-emerald-300">실측농도</th>
                    <th className="p-3 text-red-300 font-bold">보정농도</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-600 bg-slate-800">
                  {savedData.map((data, index) => (
                    <tr key={index} className="hover:bg-slate-750 transition-colors">
                      <td className="p-3 font-medium text-slate-100">{data.location}</td>
                      <td className="p-3 text-cyan-300">{data.moisturePercent}</td>
                      <td className="p-3">
                          <span className={parseFloat(data.isokineticRate) >= 95 && parseFloat(data.isokineticRate) <= 105 ? "text-emerald-900 font-bold bg-emerald-400 px-2.5 py-1 rounded-md" : "text-white font-bold bg-red-500 px-2.5 py-1 rounded-md"}>
                           {data.isokineticRate}
                          </span>
                      </td>
                      <td className="p-3 text-teal-300">{data.dustWeight}</td>
                      <td className="p-3 font-bold text-white">{data.actualConcentration}</td>
                      <td className="p-3 text-red-200 font-bold text-lg">{data.correctedConcentration}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
